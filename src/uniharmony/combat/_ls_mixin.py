...
class LocationAndScaleMixin:
    ...
    def fit_ls_model(
        ...
        # Extract site-only design matrix (remove covariate columns)
        # The first _n_sites columns of design are site indicator variables (one-hot encoded)
        site_design = design[:, : self._n_sites]

        # Estimate location parameters (gamma_hat) via OLS to
        # check how much each site's mean differs from the grand mean
        ...
        # Estimate scale parameters (delta_hat) per site to
        # check each site's variance for each feature
        # If sites have different variances, this captures it
        # We compute this per-site using sample indices, not via OLS as
        # variance is a second-order moment estimated directly
        delta_hat = []
        for idx, site_idxs in enumerate(idx_per_site):
            if self.mean_only:
                # Assume equal variance across sites
                ...
                # Estimate site-specific variances
    ...
    def _find_ls_adjustments(
        ...
        """Find L/S adjustments.

        It can use empirical Bayes to perform either parametric or non-parametric adjustments; or a simple one.

        Parameters
        ----------
        gamma_hat : array, shape (n_sites, n_features)
            Estimated location (mean) shifts for each site and feature from fit_ls_model.
        delta_hat : list of array
            Estimated scale (variance) parameters for each site and feature from fit_ls_model.
            Each array has shape (n_features,).
        data : array
            The standardized data if empirical Bayes is used else discarded.
        idx_per_site : list of list of int
            List where each element contains sample indices for that site.
        delta_epsilon : float, optional (default 1e-8)
            Small constant to add to variance to avoid numerical issues in inverse-gamma estimation.
    ...
    def _find_priors(
        ...
        # Handle zero/negative variances in delta_hat
        # Replace them with a small epsilon and log warnings
        delta_hat_clean = []
        ...
        # Compute inverse-gamma prior parameters for scale (delta)
        # Estimate a and b using method of moments from the observed delta_hat values
        # a_prior and b_prior are estimated per feature (not per site) to allow
        # feature-specific variance pooling
        if self.mean_only:
            # No scale adjustment, so no need for inverse-gamma priors
            a_prior = None
            b_prior = None
        else:
            # Estimate inverse-gamma hyperparameters for each site
    ...
    def _find_parametric_adjustments(
        ...
        ------
        ValueError
            If the computer delta_star and gamma_star have incorrect shapes.
        ...
        for i, site_idxs in enumerate(idx_per_site):
            if self.mean_only:
                ...
                delta_star.append(np.ones(data.shape[0]))
            else:
                # Adjust both location and variance iteratively
        ...
        expected_shape = (self._n_sites, data.shape[0])
    ...
    def _iteration_solver(
        ...
        Parameters
        ----------
        data : array, shape (n_features, n_samples_site)
        ...
        ------
        ValueError
            If NaN values are found in ``data``.

        """
        gamma_hat_new = None
        delta_hat_new = None
        # Handle missing data (NaN) by counting non-NaN samples per feature
        sample_size = np.sum(~np.isnan(data), axis=1)
    ...
    def _find_non_parametric_adjustments(
        ...
        for i, site_idxs in enumerate(idx_per_site):
            ...
            if self.mean_only:
                site_delta = np.ones(data.shape[0])
            else:
    ...
    def harmonize(
        ...
        This method applies the L/S adjustments (gamma_star, delta_star)
        to remove site effects and then rescales the data back to original scale.

        The harmonization formula for each site j:
            bayes_data = (data - gamma_star[j]) / sqrt(delta_star[j])
        Then rescale: bayes_data * sqrt(var_pooled) + mean

        Parameters
        ----------
        data : array, shape (n_features, n_samples)
            Standardized data from _standardize_across_features.
        mean : array, shape (n_features, n_samples)
        ...
        if not hasattr(self, "_gamma_star") or not hasattr(self, "_delta_star"):
            raise ValueError("gamma_star and delta_star must be computed before adjustment")

        # Validate shapes
        expected_shape = (self._n_sites, data.shape[0])
        if self._gamma_star.shape != expected_shape:
            raise ValueError(f"gamma_star shape {self._gamma_star.shape} != expected {expected_shape}.")
        if self._delta_star.shape != expected_shape:
            raise ValueError(f"delta_star shape {self._delta_star.shape} != expected {expected_shape}.")
        if self._var_pooled.shape[0] != data.shape[0]:
            raise ValueError(f"var_pooled has {self._var_pooled.shape[0]} features but data has {data.shape[0]} features.")

        # Apply EB adjustments per site
        ...
        for j, site_idxs in enumerate(idx_per_site):
            ...
            # Get adjusted parameters for this site
            site_gamma = self._gamma_star[j, :][:, np.newaxis]  # (n_features, 1)
            site_delta = np.maximum(self._delta_star[j, :], epsilon)[:, np.newaxis]  # (n_features, 1)

            # Apply harmonization formula
            bayes_data[:, site_idxs] = (bayes_data[:, site_idxs] - site_gamma) / np.sqrt(site_delta)

        # Rescale to original scale
        bayes_data = bayes_data * pooled_std + mean
